...
export function useP2P(sessionId: string, localStream: MediaStream | null) {
  ...
  useEffect(() => {
    ...
    // For local development, point to your signaling server.
    // In a real deployment, this would be your public server URL.
    ...
    socketRef.current.on('user joined', (payload) => {
      ...
      if(peer && socketRef.current) {
        ...
        socketRef.current.emit("returning signal", { signal: payload.signal, callerID: socketRef.current.id, userToSignal: payload.callerID });
      }
    });
